...
class ActionServer extends EventEmitter {
  ...
  publishStatus() {
    ...
    for (let i = 0, len = this._goalHandleList.length; i < len; ++i) {
      ...
      statusList.push(goalHandle.getGoalStatus());
...
let log, ActionServerInterface;

ActionConfig.on('configured', function(config) {
  log = config.log;

  GoalStatusMsg = config.messages.getMessage('actionlib_msgs/msg/GoalStatus');
  GoalStatuses = config.messages.getMessageConstants('actionlib_msgs/msg/GoalStatus');
  GoalIdMsg = config.messages.getMessage('actionlib_msgs/msg/GoalId');
  GoalStatusArrayMsg = config.messages.getMessage('actionlib_msgs/msg/GoalStatusArray');
